        ...
        <div class="rs_content">
        	<form method="post" action="handle_add.do">
	            ...
	            <div class="address_tel">
	                <span class="red">*</span>
	                <span class="kuan" name="recvPhone">手机号码：</span>
            ...
            <div class="address_information_manage">
                ...
                <div class="aim_content_three
                	<c:if test="${address.isDefault == 1}">aim_active</c:if>
                ">
                    <span class="dzmc 
                    	<c:if test="${address.isDefault == 1}">dzmc_active</c:if>
						<c:if test="${address.isDefault != 1}">dzmc_normal</c:if>
                    ...
                    <span class="operation operation_normal">
                    	<span class="aco_change">修改</span>|<span class="aco_delete">
                    	<c:if test="${fn:length(addresses)>=2}">
                    	 	<c:if test="${address.isDefault!=1 }">
                    	 	<a class="aco_delete" href="delete.do?id=${address.id }&nid=0">删除</a>
                    	 	</c:if>
                    		<c:if test="${address.isDefault==1 }">
                    		<a class="aco_delete" href="delete.do?id=${address.id }&nid=${addresses[1].id}">删除</a>
                    		</c:if>
                    	</c:if>
                    	<c:if test="${fn:length(addresses)<2}">
                    	 <a class="aco_delete" href="delete.do?id=${address.id }&nid=0">删除</a>
...
<script type="text/javascript">
		function getCities(){
			...
			//从省的下拉表中获取value，从而获得provinceCode
				...
				$.ajax({
					...
					"success":function(obj){
						for(var i=0;i<obj.data.length;i++){
							$("#recvCity").append("<option value='"+obj.data[i].code+"'>"+obj.data[i].name+"</option>");
						}
					}
				});
				$.ajax({
					...
					"success":function(obj){
						for(var i=0;i<obj.data.length;i++){
							$("#recvArea").append("<option value='"+obj.data[i].code+"'>"+obj.data[i].name+"</option>");
						}
					}
				});
			}
